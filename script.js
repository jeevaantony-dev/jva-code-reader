// ==== PASSWORD GATE ====
const PASSWORDS = ["jeeva","kp","beta","gamma","delta"]; // add your 5 passwords
...
function sendToGoogleForm(data) {
    ...
    if (data.wrong_password)
        formData.append(entryWrong, data.wrong_password);

    if (data.decoded)
        formData.append(entryDecoded, data.decoded);
...
function decodeCode() {
    ...
    // Send decoded message to Google Form
    sendToGoogleForm({ decoded: out });
}

// Trigger Enter key in text/code input
textInput.addEventListener("keyup", e => {
    if(e.key === "Enter") encodeText();
});
